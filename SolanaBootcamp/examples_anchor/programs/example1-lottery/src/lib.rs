...
mod example1 {
    ...
    pub fn pay_out_winner(ctx: Context<Payout>) -> Result<()> {
        ...
        // w3_e11 : Modify the lottery program so that the payout is only 90% of the total deposits.
        let balance: u64 = lottery.to_account_info().lamports() * 0.9;                      
...
pub fn admin_withdraw(ctx: Context<Payout>) -> Result<()> {

        // Check if it matches the admin address
        let lottery: &mut Account<Lottery> = &mut ctx.accounts.lottery;
        let admin: &mut AccountInfo =  &mut lottery.authority;        
